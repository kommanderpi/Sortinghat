...
The current test covers categorical Hardware (`−1`), Bridge (`0`), and Software (`+1`) mappings; removal and migration of legacy instructor weights; the 28 Hardware / 51 Software directional capacities; the centered global question-balance calculation; the Questions-as-is marker and its accessible label; signed and directional score arithmetic; response confidence; and the Page 2 worked example. It also verifies that Page 1 always uses the raw `1×` / `1×` questions-as-is signal and does not rerender when the slider moves. The raw Page 1 graph tests its Hardware / Bridge / Software counts, median, stable empty state, accessible list semantics, and collision lanes. The Page 2 preview tests its left-to-right `0` / `50` / `100` positions, vertical collision lanes, and placement directly after the balance control. It also checks the Page 2 single-column flow: Cohort strategy precedes Scoring maths, all four panels span the controls row, and the document references both browser assets with `?v=5`. The remaining checks cover the conversation heading, complete numerator and denominator terms, and the concrete evidence example. All mapped Page 2 sources on the same side receive the same global multiplier, while manual direct position remains unadjusted. Slider changes invalidate teams and the decision log, refresh the four adjusted views, and persist once. A divergent fixture reverses a pair’s raw and adjusted ordering to confirm that Balanced metrics and Skill-spectrum ranking use the adjusted Page 2 score. The test also covers both cohort strategies, sort-map layout, and CSV import recognition.

For a browser check, first note the Page 1 raw signal graph above the roster: it should plot every loaded student, show Hardware / Bridge / Software counts and a raw median, and remain unchanged when the roster search is used or the Page 2 slider moves. Narrow the browser window with several students at similar positions and confirm their labels use separate rows instead of overlapping. Visit Page 2 and verify that the Hardware / Software question-balance slider starts at `50 / 50`. Its summary should report Hardware `×1.41` and Software `×0.77` (rounded), and the marked Questions-as-is position should read `35.4 / 64.6`. Immediately below the slider, confirm the adjusted preview is horizontal from Hardware `0` through Bridge `50` to Software `100`. The next full-width panel should be Cohort strategy; the full-width Scoring maths panel should follow it. Open the maths panel and confirm its four stages, complete optional manual terms, and the `3` Hardware / `1` Software evidence example ending at position `25`. Confirm the worked-example heading reads **A student’s conversation with the Hat**. Move the control and confirm that its accessible value text, multiplier summary, Page 2 preview, worked-example rows, scoring status, and results change together, while the Page 1 raw graph, signal, and evidence labels remain unchanged. Existing cohort results should clear; run either strategy again and inspect the decision trail.

## Static-asset cache version

[index.html](../index.html) currently loads `styles.css?v=5` and `app.js?v=5`. These versioned URLs help a static deployment request the matching stylesheet and script after an update. If either asset reference is intentionally versioned again, update the two references together and rerun the smoke test; it asserts the current `?v=5` values.
